...
const RelatedProducts = ({ category, productId }) => {
    ...
    useEffect(() => {
        if (products.length > 0 && category) {
            const productsData = products.filter((doc) => doc.category === category && doc._id !== productId)
            setRelDoc(productsData)
        }
        ...
        <div className='flex flex-col items-center gap-4 my-16 text-gray-900'>
            ...
            <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                {relDoc.map((item, index) => (
                    <div onClick={() => { navigate(`/product/${item._id}`); window.scrollTo(0, 0) }} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
